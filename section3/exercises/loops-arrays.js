...
var creatures = ["squirrel", "mouse", "spider", "raccoon"];

for (var i = 0; i < creatures.length; i++) {
  console.log("The " + creatures[i] + " is awesome!");
}

// YOU DO: Create an array of foods and then iterate over that array to log "Add
// <food> to shopping list" for each food item.

var groceries = new Array("seltzer", "pasta", "cereal", "creamer", "bananas");

for (var i = 0; i < groceries.length; i++){
  console.log("Add " + groceries[i] + " to shopping list");
}

// YOU DO: Create an array of numbers and then iterate over that array to log double
// of each of the number. (Ex: 4 doubled is 8.)

var num = [1, 2, 3, 5, 8, 13, 21, 34, 55];
...
for (var i = 0; i < ages.length; i++) {
  if (ages[i] < 18) {
    console.log("Oh no, you can't vote quite yet."/* + ages[i] */);
  } else {
