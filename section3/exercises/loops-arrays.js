...
var shoppingList = ["bread", "rice", "peanut butter"];

for (var i = 0; i < shoppingList.length; i++) {
  console.log("Add " + shoppingList[i] + " to the shopping list");
}


// YOU DO: Create an array of numbers and then iterate over that array to log double
// of each of the number. (Ex: 4 doubled is 8.)
var doubleNum = [1, 10, 300, 1500, 2500, 134343343];

for (var i = 0; i < doubleNum.length; i++) {
  console.log("Double " + doubleNum[i] + " = " + doubleNum[i] * 2);
}
...
//var i = 2 starts at 2 to skip over the first (0) and second (1) positions in the array
for (var i = 2; i < names.length; i++) {
  console.log(i + " Happy Birthday " + names[i]);
}
...
ages.sort()
//used sort method to make this a bit easier to read

for (var i = 0; i < ages.length; i++) {
  if (ages[i] < 18) { console.log("Oh no, you can't vote quite yet. " + ages[i]);
  //included age above as quality control; remove from final coding
} else {
  console.log("Make sure you get out and vote! " + ages[i]);
  //included age above as quality control; remove from final coding
}
}
